...
namespace VisualTests.Runners.Common
{
    public abstract class VisualTestDefinition : IDisposable
    {
        ...
        public void Initialize(SurfaceTypes surfaceType, string prefferedAudio, object context = null)
        {
            this.assetsDirectory = new AssetsDirectory(this.assetsRootPath);

            switch (surfaceType)
            {
                case SurfaceTypes.Forms:
                    this.windowSystem = GetInstance<WindowsSystem>("WaveEngine.Forms", "FormsWindowsSystem");
                    break;
            ...
            if (prefferedAudio != null)
            {
                if (prefferedAudio == "XAudio2")
                {
                    this.audioDevice = this.GetInstance<AudioDevice>("WaveEngine.XAudio2", "XAudioDevice");
                }
                else if (prefferedAudio == "OpenAL")
                {
                    this.audioDevice = this.GetInstance<AudioDevice>("WaveEngine.OpenAL", "ALAudioDevice");
                }
                else
                {
                    throw new InvalidOperationException("Invalid audio backend");
                }
            }
        }
